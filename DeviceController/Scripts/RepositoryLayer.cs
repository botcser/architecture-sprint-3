...
namespace MetricsController.Scripts
{
    public class RepositoryLayer : IMetricaRepository
    {
        ...
        public bool UpdateDeviceStatus(ulong deviceId, string status)
        {
            var oldMetric = Load(deviceId);

            if (oldMetric != null)
            {
                oldMetric.Status = status;
                _metricsTable.ReplaceOne(i => i.deviceId == oldMetric.DeviceId, new metric(oldMetric));
            }
            else
            {
